...
function estandar4_meses_programados_texto(array $programacion, int $anio): string
{
    ...
    $seleccionados = [];
    foreach ($programacion as $mes => $detalle) {
        $mes = (int)$mes;
        if (isset($meses[$mes])) {
            $texto_mes = $meses[$mes];
            $fecha_programada = is_array($detalle) ? (string)($detalle['fecha_programada'] ?? '') : '';
            $hora_programada = is_array($detalle) ? (string)($detalle['hora_programada'] ?? '') : '';
            if ($fecha_programada !== '') {
                $texto_mes .= ' ' . date('d/m/Y', strtotime($fecha_programada));
            }
            if ($hora_programada !== '') {
                $texto_mes .= ' ' . $hora_programada;
            }
            $seleccionados[] = $texto_mes;
        }
    }
...
try {
    ...
    if ($accion === 'guardar_actividad') {
        ...
        $fechas_programadas = is_array($_POST['mes_fecha'] ?? null) ? $_POST['mes_fecha'] : [];
        $horas_programadas = is_array($_POST['mes_hora'] ?? null) ? $_POST['mes_hora'] : [];
        $programacion = [];
        $programacion_actual = [];
        if ($actividad_id > 0) {
        ...
        foreach (array_unique($meses) as $mes) {
            if ($mes >= 1 && $mes <= 12) {
                $detalle_mes = $programacion_actual[(string)$mes]
                    ?? ['estado' => 'P', 'fecha' => null];
                if (!is_array($detalle_mes)) {
                    $detalle_mes = ['estado' => $detalle_mes ?: 'P', 'fecha' => null];
                }

                $fecha_programada = trim((string)($fechas_programadas[$mes] ?? ''));
                $hora_programada = trim((string)($horas_programadas[$mes] ?? ''));
                if ($fecha_programada !== '' && preg_match('/^\d{4}-\d{2}-\d{2}$/', $fecha_programada) !== 1) {
                    throw new RuntimeException('La fecha programada no es vÃ¡lida.');
                }
                if ($hora_programada !== '' && preg_match('/^\d{2}:\d{2}$/', $hora_programada) !== 1) {
                    throw new RuntimeException('La hora programada no es vÃ¡lida.');
                }

                $detalle_mes['fecha_programada'] = $fecha_programada !== '' ? $fecha_programada : null;
                $detalle_mes['hora_programada'] = $hora_programada !== '' ? $hora_programada : null;
                $programacion[(string)$mes] = $detalle_mes;
            }
        }
