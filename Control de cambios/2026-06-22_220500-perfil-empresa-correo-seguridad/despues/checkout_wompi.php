...
function sendWelcomeEmail(string $toEmail, string $toName, string $cedula, string $planNombre): array {
    ...
    $fromAddress = getenv('SMTP_FROM_ADDRESS') ?: $user;
    $fromName = getenv('SMTP_FROM_NAME') ?: 'SG-SST Pro';

    if ($user === '' || $pass === '') return [false, 'Faltan credenciales SMTP'];
    if ($fromAddress === '') return [false, 'Falta SMTP_FROM_ADDRESS'];
...
$correo_error = '';
$cuenta_activa = ($solicitud['estado'] === 'aprobada');
            ...
            if (isset($datos_wompi['data']['status'])) {
                ...
                if ($estado_pago === 'APPROVED' && $solicitud['estado'] === 'pendiente') {
                    ...
                    try {
                        ...
                        // B. Creamos o actualizamos el usuario Representante.
                        // Si el correo o cedula ya existen, no se debe romper la activacion del pago.
                        $stmt_user = $conn->prepare("SELECT id FROM usuarios WHERE cedula = ? OR email = ? LIMIT 1");
                        $stmt_user->execute([$solicitud['cedula'], $solicitud['email']]);
                        $usuario_existente = $stmt_user->fetch(PDO::FETCH_ASSOC);

                        if ($usuario_existente) {
                            $stmt_user_upd = $conn->prepare("
                                UPDATE usuarios
                                SET empresa_id = ?, nombre = ?, apellido = ?, cedula = ?, email = ?, telefono = ?,
                                    rol = 'representante', direccion = ?, ciudad = ?, barrio = ?, localidad = ?,
                                    firma = ?, activo = 1, correo_seguridad = ?,
                                    nombre_empresa = ?, tipo_doc_empresa = 'NIT', num_doc_empresa = ?, clase_riesgo = ?
                                WHERE id = ?
                            ");
                            $stmt_user_upd->execute([
                                ...
                                $solicitud['firma'], $solicitud['email'],
                                $solicitud['empresa_nombre'] ?: $solicitud['nombre'],
                                $solicitud['empresa_nit'] ?: $solicitud['cedula'],
                                $solicitud['empresa_clase_riesgo'] ?? null,
                            ...
                            $stmt_ins = $conn->prepare("
                                INSERT INTO usuarios (
                                    ...
                                    correo_seguridad, direccion, ciudad, barrio, localidad, firma, activo,
                                    nombre_empresa, tipo_doc_empresa, num_doc_empresa, clase_riesgo
                                ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?, 1,?,?,?,?)
                            ");
                            $stmt_ins->execute([
                                $solicitud_id, $solicitud['nombre'], $solicitud['apellido'], $solicitud['cedula'],
                                $solicitud['email'], $solicitud['telefono'], 'representante',
                                $solicitud['email'], $solicitud['direccion'], $solicitud['ciudad'], $solicitud['barrio'],
                                $solicitud['localidad'], $solicitud['firma'],
                                $solicitud['empresa_nombre'] ?: $solicitud['nombre'], 'NIT',
                                $solicitud['empresa_nit'] ?: $solicitud['cedula'],
                                $solicitud['empresa_clase_riesgo'] ?? null
                            ]);
                        }
                        ...
                        $conn->rollBack();
                        error_log('Error activando solicitud Wompi #' . $solicitud_id . ': ' . $e->getMessage());
                    }
                }

                if ($estado_pago === 'APPROVED' && $cuenta_activa) {
                    $welcomeSessionKey = 'welcome_email_sent_' . $solicitud_id;
                    if (empty($_SESSION[$welcomeSessionKey])) {
                        ...
                        if ($ok) {
                            $correo_enviado = true;
                            $_SESSION[$welcomeSessionKey] = true;
                        } else {
                            $correo_error = $msg;
                            error_log('No se pudo enviar el correo de bienvenida para solicitud #' . $solicitud_id . ': ' . $msg);
                        }
                    } else {
                        $correo_enviado = true;
                    }
                ...
                if ($estado_pago === 'APPROVED' && !$cuenta_activa) {
                    $estado_pago = 'ERROR_ACTIVATION';
                    error_log('Pago aprobado por Wompi, pero la solicitud #' . $solicitud_id . ' no pudo activarse en la base de datos.');
                }
            } else {
                $estado_pago = 'ERROR';
            }
        ...
        <div class="content-wrapper">
            ...
            <?php if ($transaccion_id): ?>
                ...
                <div class="status-wrapper">
                    ...
                    <div class="status-header">
                        <h1><?php echo ($estado_pago === 'ERROR_ACTIVATION') ? 'Pago aprobado, activacion pendiente' : 'El pago no se pudo procesar'; ?></h1>
                        <?php if($estado_pago === 'ERROR_ACTIVATION'): ?>
                            <p>El banco aprobo la transaccion, pero la plataforma no pudo terminar la activacion automatica. No intentes pagar de nuevo; revisa el registro tecnico o contacta soporte.</p>
                        <?php else: ?>
