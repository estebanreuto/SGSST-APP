...
<head>
    ...
    <style>
        ...
        .rep-summary-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 12px; margin-bottom: 18px; }
        .rep-metric { border: 1px solid #e2e8f0; background: #f8fafc; border-radius: 10px; padding: 14px; min-height: 86px; display: flex; flex-direction: column; justify-content: space-between; }
        .rep-metric span { color: #64748b; font-size: 0.66rem; font-weight: 800; text-transform: uppercase; letter-spacing: 0.04em; }
        .rep-metric strong { color: var(--blue-dark); font-size: 1.25rem; font-weight: 850; margin-top: 8px; }
        .rep-metric.ok strong { color: #15803d; }
        .rep-metric.warn strong { color: #b45309; }
        .rep-table-wrap { overflow-x: auto; border: 1px solid #e2e8f0; border-radius: 12px; background: #fff; }
        .rep-summary-table { width: 100%; border-collapse: collapse; min-width: 860px; }
        .rep-summary-table th { background: #f8fafc; color: #475569; font-size: 0.68rem; text-transform: uppercase; letter-spacing: 0.05em; text-align: left; padding: 12px; border-bottom: 1px solid #e2e8f0; }
        .rep-summary-table td { padding: 13px 12px; border-bottom: 1px solid #eef2f7; color: #334155; font-size: 0.78rem; vertical-align: top; }
        .rep-summary-table tr:last-child td { border-bottom: none; }
        .summary-status { display: inline-flex; align-items: center; gap: 6px; padding: 5px 9px; border-radius: 999px; font-size: 0.64rem; font-weight: 850; text-transform: uppercase; }
        ...
        .risk-pill { display: inline-flex; padding: 4px 8px; border-radius: 999px; background: #eef2ff; color: #1e3a8a; font-size: 0.66rem; font-weight: 800; margin: 2px 4px 2px 0; }
                            ...
                            <?php else: ?>
                                <div class="info-pila-box" style="background:#f8fafc; margin-bottom:0;">
                                    <p style="font-weight:800; color:var(--blue-dark); margin-bottom:8px;">
                                        <i class="fa-solid fa-eye" style="color:var(--primary2);"></i>
                                        Vista ejecutiva del Representante Legal
                                    </p>
                                    <p style="line-height:1.55; color:#475569;">
                                        El Responsable SST carga y valida los soportes PILA. En este rol solo se presenta el estado de cumplimiento, valores y resumen anual para facilitar la revisión gerencial.
                                    </p>
                                </div>
                            <?php endif; ?>
                        <?php endif; ?>
                                ...
                                <tbody>
                                    <?php for ($m = 1; $m <= 12; $m++): ?>
                                        <?php
                                            ...
                                            $estado_texto = $is_loaded ? 'Al dia' : ($esta_vencido ? 'Pendiente vencida' : 'Pendiente');
                                            $riesgos = $p['riesgos_detectados'] ?? '';
                                        ?>
                                        <tr>
                                            <td><strong style="color:var(--blue-dark);"><?php echo $mes_nombre; ?></strong></td>
                                            <td>
                                            ...
                                            <td><?php echo htmlspecialchars($limite); ?></td>
                                            <td>
