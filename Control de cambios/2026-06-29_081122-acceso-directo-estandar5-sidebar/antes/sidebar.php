...
<style>
    ...
    .sub-item.has-children { cursor: pointer; }
                        ...
                        <?php for($i = 1; $i <= 7; $i++): ?>
                            <?php if ($i === 5): ?>
                                <div class="standard-submodules <?php echo $estandar5_activo ? 'active' : ''; ?>">
                                    <button type="button" class="sub-item std-item has-children <?php echo $estandar5_activo ? 'active' : ''; ?>" data-search="<?php echo $i . ' ' . strtolower(htmlspecialchars($nombres_estandares[$i] . ' ' . implode(' ', $estandar5_submodulos))); ?>">
...
<script>
    ...
    document.querySelectorAll('.standard-submodules .has-children').forEach(toggle => {
        toggle.addEventListener('click', function(event) {
            event.preventDefault();
            this.closest('.standard-submodules').classList.toggle('active');
        });
    });
