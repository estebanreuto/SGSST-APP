...
require_once __DIR__ . "/config/sst_license_options.php";
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!isset($_POST["autorizacion"]) || $_POST["autorizacion"] !== "si") {
        ...
        $empresa_nombre_registro = trim($_POST["empresa_nombre_registro"] ?? '');
        $empresa_nit_registro = trim($_POST["empresa_nit_registro"] ?? '');
        $empresa_clase_riesgo = trim($_POST["empresa_clase_riesgo"] ?? '');
        $clases_riesgo_validas = ['I', 'II', 'III', 'IV', 'V'];
        ...
        if ($rol === 'representante') {
            if (empty($plan_id)) {
                ...
                $stmt_usuario_existente = $conn->prepare("SELECT id FROM usuarios WHERE cedula = ? OR email = ? LIMIT 1");
                $stmt_usuario_existente->execute([$_POST["cedula"], $_POST["email"]]);

                if ($stmt_usuario_existente->fetch()) {
                    $error_msg = "Ya existe una cuenta activa con esta cedula o correo. Ingresa al sistema o usa datos diferentes para una nueva empresa.";
                }
            }

            if (empty($error_msg)) {
                if ($empresa_nombre_registro === '') {
                    $error_msg = "Ingresa el nombre o razon social de la empresa.";
                } elseif ($empresa_nit_registro === '') {
                    $error_msg = "Ingresa el NIT de la empresa.";
                } elseif (!in_array($empresa_clase_riesgo, $clases_riesgo_validas, true)) {
                    $error_msg = "Selecciona el nivel de riesgo ARL de la empresa.";
                }
            }
            ...
            if (empty($error_msg)) {
                $sql = "INSERT INTO solicitudes_empresas (
                    ...
                    empresa_nombre, empresa_nit, empresa_clase_riesgo,
                ...
                $stmt->execute([
                    ...
                    $_POST["telefono"], $empresa_nombre_registro, $empresa_nit_registro, $empresa_clase_riesgo,
                    $_POST["direccion"] ?? null, $_POST["ciudad"] ?? null,
        ...
        // =========================
            ...
            if (empty($empresa_nit)) {
                ...
                $stmt_buscar = $conn->prepare("SELECT id FROM solicitudes_empresas WHERE (empresa_nit = ? OR cedula = ?) AND estado = 'aprobada' LIMIT 1");
                $stmt_buscar->execute([$empresa_nit, $empresa_nit]);
                ...
                if (!$empresa_data) {
                    ...
                    if (!$plan_empresa || $plan_empresa['estado'] !== 'aprobada') {
                        ...
                        if ($rol === 'sst' && empty($error_msg)) {
                            $licencia_sst = $_POST["licencia"] ?? 'no';
                            $tipo_licencia = $_POST["tipoLicencia"] ?? '';
                            $numero_licencia = trim($_POST["numLicencia"] ?? '');
                            $fecha_licencia = $_POST["fechaLicencia"] ?? '';
                            $expedida_por = $_POST["expedida"] ?? '';

                            if ($licencia_sst !== 'si') {
                                $error_msg = "El Responsable SST debe registrar una licencia SST vigente.";
                            } elseif ($licencia_sst === 'si') {
                                if (!array_key_exists($tipo_licencia, SST_LICENSE_TYPES)) {
                                    $error_msg = "Selecciona un tipo de licencia SST valido.";
                                } elseif ($numero_licencia === '') {
                                    $error_msg = "Ingresa el numero de licencia SST.";
                                } elseif ($fecha_licencia === '') {
                                    $error_msg = "Ingresa la fecha de expedicion de la licencia SST.";
                                } elseif (sst_license_is_future_date($fecha_licencia)) {
                                    $error_msg = "La fecha de expedicion de la licencia SST no puede ser futura.";
                                } elseif (sst_license_is_expired($fecha_licencia)) {
                                    $error_msg = "La licencia SST ya cumplio 10 anos de vigencia. Debe renovarse antes del registro.";
                                } elseif (!in_array($expedida_por, SST_LICENSE_ISSUERS, true)) {
                                    $error_msg = "Selecciona la secretaria de salud que expidio la licencia SST.";
                                }
                            }
                        }
                        ...
                        if (empty($error_msg)) {
                            ...
                            $stmt->execute([
                                ...
                                $licencia_sst ?? ($_POST["licencia"] ?? null), $tipo_licencia ?? ($_POST["tipoLicencia"] ?? null), $numero_licencia ?? ($_POST["numLicencia"] ?? null),
                                $fecha_licencia ?? ($_POST["fechaLicencia"] ?? null), $expedida_por ?? ($_POST["expedida"] ?? null), $_POST["direccion"] ?? null,
                            ...
                            if ($rol === "trabajador") {
                                ...
                                if (is_array($tiempo_libre)) {
                                    $tiempo_libre = implode(', ', array_filter(array_map('trim', $tiempo_libre)));
                                } else {
                                    $tiempo_libre = trim((string)$tiempo_libre);
                                }
                                $enfermedad = trim((string)($_POST["enfermedad"] ?? ''));
                                if ($enfermedad === 'Otras') {
                                    $detalle_enfermedad = trim((string)($_POST["enfermedad_otro"] ?? ''));
                                    $enfermedad = $detalle_enfermedad !== '' ? 'Otras: ' . $detalle_enfermedad : 'Otras';
                                }
                                $turno = trim((string)($_POST["turno"] ?? ''));
                                if ($turno === 'Otro horario') {
                                    $dias_turno = $_POST["turno_dias"] ?? [];
                                    if (is_array($dias_turno)) {
                                        $dias_turno = implode(', ', array_filter(array_map('trim', $dias_turno)));
                                    } else {
                                        $dias_turno = trim((string)$dias_turno);
                                    }
                                    $hora_inicio = trim((string)($_POST["turno_inicio"] ?? ''));
                                    $hora_salida = trim((string)($_POST["turno_salida"] ?? ''));
                                    $detalle_turno = trim($dias_turno . ' ' . $hora_inicio . ' - ' . $hora_salida);
                                    $turno = $detalle_turno !== ' -' ? 'Otro horario: ' . $detalle_turno : 'Otro horario';
                                }
                                ...
                                $stmt2->execute([
                                    ...
                                    $_POST["escolaridad"], $_POST["vivienda"], $tiempo_libre, $_POST["experiencia"],
                                    $_POST["estrato"], $_POST["convive_con"], $_POST["raza"], $_POST["tipo_contrato"],
                                    $turno, $_POST["antiguedad"], $enfermedad, $_POST["fuma"],
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        #trabajador .field label { text-transform: none; letter-spacing: 0; font-size: .82rem; font-weight: 700; }
        ...
        .multi-toggle { width: 100%; min-height: 45px; justify-content: space-between; padding: 12px 14px 12px 40px; border: 1px solid var(--border); border-radius: 10px; background: #ffffff; color: var(--text-main); font-size: .9rem; font-weight: 500; text-align: left; box-shadow: none; }
        .multi-toggle:hover, .multi-toggle:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(255, 138, 31, .14); transform: none; outline: none; }
        .multi-toggle .multi-value { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        .multi-menu { display: none; position: absolute; left: 0; right: 0; top: calc(100% + 4px); z-index: 30; background: #ffffff; border: 1px solid var(--border); border-radius: 10px; box-shadow: 0 14px 30px rgba(15, 23, 42, .16); padding: 8px; }
        .multi-control.open .multi-menu { display: grid; gap: 4px; }
        .native-multi-source { display: none; }
        .multi-option { display: flex; align-items: center; gap: 10px; padding: 9px 10px; border-radius: 8px; color: var(--text-main); font-size: .86rem; font-weight: 600; cursor: pointer; }
        .multi-option:hover { background: #fff7ed; color: var(--primary2); }
        .multi-option input { width: 16px; height: 16px; padding: 0; border-radius: 4px; accent-color: var(--primary2); flex: 0 0 auto; }
        .multi-control.open .icon { opacity: .75; color: var(--primary); }
        .other-disease-control { margin-top: 10px; }
        .turno-extra { margin-top: 10px; display: grid; gap: 10px; }
        .turno-days { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px; padding: 10px; border: 1px solid var(--border); border-radius: 10px; background: #ffffff; }
        .turno-day { display: flex !important; align-items: center; gap: 8px; margin: 0 !important; color: var(--text-main) !important; font-size: .78rem !important; font-weight: 650 !important; cursor: pointer; }
        .turno-day input { width: 15px; height: 15px; padding: 0; accent-color: var(--primary2); flex: 0 0 auto; }
        .turno-time-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px; }
        @media(max-width: 600px) {
            .turno-days, .turno-time-grid { grid-template-columns: 1fr; }
        }
...
<body>
            ...
            <?php else: ?>
                    ...
                    <form method="POST" id="mainForm" onsubmit="return beforeSubmit()">
                        ...
                        <div class="grid">
                            ...
                            <div class="field full-width hidden" id="empresa_nueva_section">
                                <div class="section-title" style="margin-bottom: 12px;">
                                    <h3>Datos de la Empresa</h3>
                                    <span class="badge">Representante Legal</span>
                                </div>
                                <div class="grid">
                                    <div class="field">
                                        ...
                                        <div class="control">
                                            <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                <path d="M3 21h18" /><path d="M5 21V7l7-4 7 4v14" /><path d="M9 9h6M9 13h6M9 17h6" />
                                            </svg>
                                            <input name="empresa_nombre_registro" id="empresa_nombre_registro" value="<?php echo htmlspecialchars($_POST['empresa_nombre_registro'] ?? ''); ?>" placeholder="Ej. Constructora ABC S.A.S.">
                                        </div>
                                    </div>
                                    <div class="field">
                                        ...
                                        <div class="control">
                                            ...
                                            <input name="empresa_nit_registro" id="empresa_nit_registro" value="<?php echo htmlspecialchars($_POST['empresa_nit_registro'] ?? ''); ?>" placeholder="Ej. 900123456-7">
                                        </div>
                                    </div>
                                    <div class="field">
                                        ...
                                        <div class="control">
                                            ...
                                            <select name="empresa_clase_riesgo" id="empresa_clase_riesgo">
                                                <option value="">Selecciona el nivel...</option>
                                                <option value="I" <?php echo (($_POST['empresa_clase_riesgo'] ?? '') === 'I') ? 'selected' : ''; ?>>Riesgo I (Minimo)</option>
                                                <option value="II" <?php echo (($_POST['empresa_clase_riesgo'] ?? '') === 'II') ? 'selected' : ''; ?>>Riesgo II (Bajo)</option>
                                                <option value="III" <?php echo (($_POST['empresa_clase_riesgo'] ?? '') === 'III') ? 'selected' : ''; ?>>Riesgo III (Medio)</option>
                                                <option value="IV" <?php echo (($_POST['empresa_clase_riesgo'] ?? '') === 'IV') ? 'selected' : ''; ?>>Riesgo IV (Alto)</option>
                                                <option value="V" <?php echo (($_POST['empresa_clase_riesgo'] ?? '') === 'V') ? 'selected' : ''; ?>>Riesgo V (Maximo)</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="grid">
                                <div class="field">
                                    ...
                                    <div class="control">
                                        ...
                                        <select name="licencia" id="licenciaSst">
                                            <option value="no" <?php echo (($_POST['licencia'] ?? '') === 'no') ? 'selected' : ''; ?>>No</option>
                                            <option value="si" <?php echo (($_POST['licencia'] ?? 'si') === 'si') ? 'selected' : ''; ?>>Si</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="field">
                                    ...
                                    <div class="control">
                                        ...
                                        <select name="tipoLicencia" id="tipoLicencia">
                                            ...
                                            <?php foreach (SST_LICENSE_TYPES as $value => $label): ?>
                                                <option value="<?php echo htmlspecialchars($value); ?>" <?php echo (($_POST['tipoLicencia'] ?? '') === $value) ? 'selected' : ''; ?>>
                                ...
                                <div class="field">
                                    ...
                                    <div class="control">
                                        ...
                                        <input name="numLicencia" id="numLicencia" value="<?php echo htmlspecialchars($_POST['numLicencia'] ?? ''); ?>">
                                    </div>
                                </div>
                                ...
                                <div class="field">
                                    ...
                                    <div class="control">
                                        ...
                                        <input type="date" name="fechaLicencia" id="fechaLicencia" value="<?php echo htmlspecialchars($_POST['fechaLicencia'] ?? ''); ?>" max="<?php echo date('Y-m-d'); ?>">
                                    </div>
                                    <div id="fechaLicenciaFeedback" style="font-size: 0.78rem; margin-top: 6px; color: #64748b;"></div>
                                </div>
                                ...
                                <div class="field full-width">
                                    ...
                                    <div class="control">
                                        ...
                                        <select name="expedida" id="expedida">
                                            <option value="">Seleccione secretaria de salud</option>
                                            <?php foreach (SST_LICENSE_ISSUERS as $issuer): ?>
                                                <option value="<?php echo htmlspecialchars($issuer); ?>" <?php echo (($_POST['expedida'] ?? '') === $issuer) ? 'selected' : ''; ?>>
                        ...
                        <div id="trabajador" class="section hidden">
                            ...
                            <div class="grid">
                                <div class="field"><label>¿Cuál es tu rango de edad?</label>
                                    <div class="control">
                                ...
                                <div class="field"><label>¿Cuál es tu estado civil?</label>
                                    <div class="control">
                                ...
                                <div class="field"><label>¿Con qué género te identificas?</label>
                                    <div class="control">
                                ...
                                <div class="field"><label>¿Cuántas personas tienes a cargo?</label>
                                    <div class="control">
                                ...
                                <div class="field"><label>¿Cuál es tu nivel de escolaridad?</label>
                                    <div class="control">
                                ...
                                <div class="field"><label>¿Qué tipo de vivienda tienes?</label>
                                    <div class="control">
                                        <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 11 12 2l9 9" /><path d="M5 10v11h14V10" /></svg>
                                ...
                                <div class="field"><label>¿Qué haces en tu tiempo libre?</label>
                                    <div class="control multi-control" id="tiempoLibreControl">
                                        ...
                                        <button type="button" class="multi-toggle" id="tiempoToggle" aria-expanded="false">
                                            <span class="multi-value">Seleccione</span>
                                            <i class="fa-solid fa-chevron-down"></i>
                                        </button>
                                        <input type="hidden" name="tiempo_libre" id="tiempoLibreInput">
                                        <select id="tiempoSelect" class="native-multi-source" multiple size="5" data-skip-required="true">
                                            <option value="">Seleccione</option>
                                ...
                                <div class="field"><label>¿Cuánta experiencia laboral tienes?</label>
                                    <div class="control">
                                ...
                                <div class="field"><label>¿Cuál es tu estrato socioeconómico?</label>
                                    <div class="control">
                                ...
                                <div class="field"><label>¿Con quién convives actualmente?</label>
                                    <div class="control">
                                ...
                                <div class="field"><label>¿Cómo te reconoces étnicamente?</label>
                                    <div class="control">
                                ...
                                <div class="field"><label>¿Qué tipo de contrato tienes?</label>
                                    <div class="control">
                                ...
                                <div class="field full-width"><label>¿Cuál es tu jornada o turno habitual?</label>
                                    <div class="control">
                                        ...
                                        <select name="turno" id="turnoSelect">
                                            ...
                                            <option>Operativo 07:00 am - 04:00 pm / Sábado medio día</option>
                                            <option>Otro horario</option>
                                        </select>
                                    </div>
                                    <div class="turno-extra hidden" id="turnoOtroControl">
                                        <div class="turno-days">
                                            <label class="turno-day"><input type="checkbox" name="turno_dias[]" value="Lunes"> Lunes</label>
                                            <label class="turno-day"><input type="checkbox" name="turno_dias[]" value="Martes"> Martes</label>
                                            <label class="turno-day"><input type="checkbox" name="turno_dias[]" value="Miercoles"> Miercoles</label>
                                            <label class="turno-day"><input type="checkbox" name="turno_dias[]" value="Jueves"> Jueves</label>
                                            <label class="turno-day"><input type="checkbox" name="turno_dias[]" value="Viernes"> Viernes</label>
                                            <label class="turno-day"><input type="checkbox" name="turno_dias[]" value="Sabado"> Sabado</label>
                                            <label class="turno-day"><input type="checkbox" name="turno_dias[]" value="Domingo"> Domingo</label>
                                        </div>
                                        <div class="turno-time-grid">
                                            <div class="control">
                                                <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10" /><path d="M12 6v6l4 2" /></svg>
                                                <input type="time" name="turno_inicio" id="turnoInicio" aria-label="Hora de inicio">
                                            </div>
                                            <div class="control">
                                                <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10" /><path d="M12 6v6l4 2" /></svg>
                                                <input type="time" name="turno_salida" id="turnoSalida" aria-label="Hora de salida">
                                            </div>
                                        </div>
                                    </div>
                                </div>

                                <div class="field"><label>¿Cuánto tiempo llevas en la empresa?</label>
                                    <div class="control">
                                ...
                                <div class="field full-width"><label>¿Sufres de alguna enfermedad diagnosticada?</label>
                                    <div class="control">
                                        <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 21s-8-4.5-8-11a5 5 0 0 1 9-3 5 5 0 0 1 9 3c0 6.5-10 11-10 11z" /></svg>
                                        <select name="enfermedad" id="enfermedadSelect">
                                            ...
                                            <option>Enfermedad pulmonar obstructiva cronica (EPOC)</option>
                                            <option>Enfermedad renal cronica</option>
                                            <option>Artrosis y trastornos osteomusculares cronicos</option>
                                            <option>Asma</option>
                                            <option>Depresion clinica</option>
                                    ...
                                    <div class="control other-disease-control hidden" id="enfermedadOtroControl">
                                        <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6h16" /><path d="M4 12h16" /><path d="M4 18h10" /></svg>
                                        <input type="text" name="enfermedad_otro" id="enfermedadOtro" placeholder="Escribe cual enfermedad o diagnostico">
                                    </div>
                                </div>

                                <div class="field"><label>¿Fumas?</label>
                                    <div class="control">
                                        ...
                                        <select name="fuma" id="fumaSelect">
                                            ...
                                            <option>Un cigarrillo al dia</option>
                                            <option>Media caja al dia</option>
                                            <option>Una caja completa al dia</option>
                                        </select>
                                    </div>
                                </div>

                                <div class="field"><label>¿Con qué frecuencia consumes alcohol?</label>
                                    <div class="control">
                                ...
                                <div class="field"><label>¿Con qué frecuencia haces actividad física?</label>
                                    <div class="control">
                                ...
                                <div class="field full-width"><label>¿Cuál describe mejor tu tipo de labor?</label>
                                    <div class="control">
                                        ...
                                        <select name="tipo_personal" id="personalSelect">
                                            ...
                                            <option>Operario de maquinaria</option>
                                            <option>Operativo conductor</option>
                                            <option>Administrativo conductor</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        ...
        function validarFechaLicencia() {
            const fechaInput = document.getElementById('fechaLicencia');
            const feedback = document.getElementById('fechaLicenciaFeedback');
            if (!fechaInput || !feedback || !fechaInput.value) {
                if (feedback) feedback.innerHTML = '';
                return true;
            }

            const expedicion = new Date(fechaInput.value + 'T00:00:00');
            const hoy = new Date();
            ...
            if (expedicion > hoy) {
                feedback.innerHTML = '<span style="color:#dc2626;">La fecha de expedicion no puede ser futura.</span>';
                return false;
            }

            if (vencimiento <= hoy) {
                feedback.innerHTML = '<span style="color:#dc2626;">Licencia vencida: ya cumplio 10 anos de vigencia.</span>';
                return false;
            }

            const formatter = new Intl.DateTimeFormat('es-CO', { year: 'numeric', month: 'long', day: 'numeric' });
            feedback.innerHTML = '<span style="color:#10b981;">Licencia vigente hasta ' + formatter.format(vencimiento) + '.</span>';
            return true;
        }

        document.getElementById('fechaLicencia')?.addEventListener('change', validarFechaLicencia);

        const tiempoLibreControl = document.getElementById('tiempoLibreControl');
        const tiempoLibreSelect = document.getElementById('tiempoSelect');
        const tiempoLibreToggle = document.getElementById('tiempoToggle');
        const tiempoLibreInput = document.getElementById('tiempoLibreInput');
        const tiempoLibreValue = tiempoLibreToggle?.querySelector('.multi-value');

        function setupTiempoLibreMulti() {
            if (!tiempoLibreControl || !tiempoLibreSelect || !tiempoLibreToggle || !tiempoLibreInput) return;

            const menu = document.createElement('div');
            menu.className = 'multi-menu';
            menu.setAttribute('aria-label', 'Opciones de tiempo libre');

            [...tiempoLibreSelect.options].forEach((option) => {
                if (!option.value && option.textContent.trim() === 'Seleccione') return;
                const label = document.createElement('label');
                label.className = 'multi-option';
                const check = document.createElement('input');
                check.type = 'checkbox';
                check.value = option.textContent.trim();
                check.dataset.optionIndex = option.index;
                if (check.value === 'Ninguno') check.dataset.exclusive = 'true';
                label.appendChild(check);
                label.append(' ' + option.textContent.trim());
                menu.appendChild(label);
            });

            tiempoLibreControl.appendChild(menu);

            function syncTiempoLibre(changed) {
                const checks = [...menu.querySelectorAll('input[type="checkbox"]')];
                if (changed?.dataset.exclusive === 'true' && changed.checked) {
                    checks.forEach((check) => { if (check !== changed) check.checked = false; });
                } else if (changed?.checked) {
                    checks.forEach((check) => { if (check.dataset.exclusive === 'true') check.checked = false; });
                }

                const selected = checks.filter((check) => check.checked).map((check) => check.value);
                [...tiempoLibreSelect.options].forEach((option) => {
                    option.selected = selected.includes(option.textContent.trim());
                });
                tiempoLibreInput.value = selected.join(', ');
                if (tiempoLibreValue) {
                    tiempoLibreValue.textContent = selected.length ? selected.join(', ') : 'Seleccione';
                }
            }

            menu.addEventListener('change', (event) => syncTiempoLibre(event.target));
            tiempoLibreToggle.addEventListener('click', () => {
                const open = !tiempoLibreControl.classList.contains('open');
                tiempoLibreControl.classList.toggle('open', open);
                tiempoLibreToggle.setAttribute('aria-expanded', open ? 'true' : 'false');
            });
            document.addEventListener('click', (event) => {
                if (!tiempoLibreControl.contains(event.target)) {
                    tiempoLibreControl.classList.remove('open');
                    tiempoLibreToggle.setAttribute('aria-expanded', 'false');
                }
            });
            syncTiempoLibre();
        }

        setupTiempoLibreMulti();

        const enfermedadSelect = document.getElementById('enfermedadSelect');
        const enfermedadOtroControl = document.getElementById('enfermedadOtroControl');
        const enfermedadOtroInput = document.getElementById('enfermedadOtro');

        function toggleEnfermedadOtra() {
            const mostrarOtra = enfermedadSelect?.value === 'Otras';
            enfermedadOtroControl?.classList.toggle('hidden', !mostrarOtra);
            if (enfermedadOtroInput) {
                enfermedadOtroInput.required = !!mostrarOtra;
                if (!mostrarOtra) enfermedadOtroInput.value = '';
            }
        }

        enfermedadSelect?.addEventListener('change', toggleEnfermedadOtra);
        toggleEnfermedadOtra();

        const turnoSelect = document.getElementById('turnoSelect');
        const turnoOtroControl = document.getElementById('turnoOtroControl');
        const turnoInicio = document.getElementById('turnoInicio');
        const turnoSalida = document.getElementById('turnoSalida');

        function toggleTurnoOtro() {
            const mostrarOtro = turnoSelect?.value === 'Otro horario';
            turnoOtroControl?.classList.toggle('hidden', !mostrarOtro);
            if (turnoInicio) turnoInicio.required = !!mostrarOtro;
            if (turnoSalida) turnoSalida.required = !!mostrarOtro;
            if (!mostrarOtro) {
                if (turnoInicio) turnoInicio.value = '';
                if (turnoSalida) turnoSalida.value = '';
                turnoOtroControl?.querySelectorAll('input[type="checkbox"]').forEach((check) => { check.checked = false; });
            }
        }

        turnoSelect?.addEventListener('change', toggleTurnoOtro);
        toggleTurnoOtro();
        ...
        function mostrar() {
            ...
            document.getElementById("empresa_nueva_section").classList.add("hidden");
            document.getElementById("seccion_planes").classList.add("hidden");
            
            const selectsTrabajador = document.querySelectorAll('#trabajador select:not([data-skip-required])');
            const camposSst = document.querySelectorAll('#sst select, #sst input');
            const inputEmpresa = document.getElementById('empresa_nit');
            const inputPlan = document.getElementById('plan_id');
            const camposEmpresaNueva = document.querySelectorAll('#empresa_nueva_section input, #empresa_nueva_section select');
            camposEmpresaNueva.forEach(c => c.removeAttribute('required'));
            ...
            if (rol === "sst" || rol === "trabajador") {
                ...
                document.getElementById("empresa_nueva_section").classList.remove("hidden");
                inputPlan.setAttribute('required', 'required');
                inputEmpresa.removeAttribute('required');
                camposEmpresaNueva.forEach(c => c.setAttribute('required', 'required'));
            ...
            if (rol === "sst") {
                ...
                camposSst.forEach(c => c.setAttribute('required', 'required'));
                validarFechaLicencia();
                ...
                camposSst.forEach(c => c.removeAttribute('required'));
            } else {
                selectsTrabajador.forEach(s => s.removeAttribute('required'));
                camposSst.forEach(c => c.removeAttribute('required'));
            }
        ...
        function beforeSubmit() { 
            ...
            if (rolSeleccionado === 'sst' && !validarFechaLicencia()) {
                document.getElementById('fechaLicencia').scrollIntoView({ behavior: 'smooth', block: 'center' });
                return false;
            }

            if (rolSeleccionado === 'sst' && document.getElementById('licenciaSst')?.value !== 'si') {
                alert('El Responsable SST debe registrar una licencia SST vigente.');
                document.getElementById('licenciaSst').scrollIntoView({ behavior: 'smooth', block: 'center' });
                return false;
            }

            if (rolSeleccionado === 'trabajador' && !document.getElementById('tiempoLibreInput')?.value) {
                alert('Selecciona al menos una opcion en Tiempo libre.');
                document.getElementById('tiempoLibreControl')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
                return false;
            }

            if (rolSeleccionado === 'trabajador' && document.getElementById('enfermedadSelect')?.value === 'Otras' && !document.getElementById('enfermedadOtro')?.value.trim()) {
                alert('Escribe el detalle de la enfermedad en la opcion Otras.');
                document.getElementById('enfermedadOtroControl')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
                document.getElementById('enfermedadOtro')?.focus();
                return false;
            }

            if (rolSeleccionado === 'trabajador' && document.getElementById('turnoSelect')?.value === 'Otro horario') {
                const diasTurno = [...document.querySelectorAll('#turnoOtroControl input[name="turno_dias[]"]:checked')];
                const inicioTurno = document.getElementById('turnoInicio')?.value;
                const salidaTurno = document.getElementById('turnoSalida')?.value;
                if (!diasTurno.length || !inicioTurno || !salidaTurno) {
                    alert('Selecciona los dias y diligencia hora de inicio y hora de salida para el otro horario.');
                    document.getElementById('turnoOtroControl')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    return false;
                }
            }
