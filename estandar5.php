...
function estandar5_evaluaciones_medicas_rows(PDO $conn, int $empresa_id): array
{
    ...
    $stmt = $conn->prepare("
        SELECT em.*, u.nombre, u.apellido, u.cedula, p.nombre_cargo, c.nombre AS centro_nombre
...
$pyp_total_trabajadores = $modulo_actual === 'promocion-prevencion' ? estandar5_pyp_total_trabajadores($conn, $empresa_id) : 0;
$pyp_rows = $modulo_actual === 'promocion-prevencion' ? estandar5_pyp_rows($conn, $empresa_id) : [];
$pyp_resumen = $modulo_actual === 'promocion-prevencion' ? estandar5_pyp_resumen($pyp_rows, $pyp_categorias, $pyp_total_trabajadores) : [];
$herramientas_grupos = estandar5_herramientas_por_grupo();
$tareas_alto_riesgo_base = estandar5_tareas_alto_riesgo();
$centros_medicos = $modulo_actual === 'perfiles-cargo' ? estandar5_centros_medicos_rows($conn, $empresa_id) : [];
$perfiles_cargo = $modulo_actual === 'perfiles-cargo' ? estandar5_perfiles_cargo_rows($conn, $empresa_id) : [];
$procesos_perfil = $modulo_actual === 'perfiles-cargo' ? estandar5_procesos_perfil_rows($conn, $empresa_id) : [];
$eval_centros_medicos = $modulo_actual === 'evaluaciones-medicas' ? estandar5_centros_medicos_rows($conn, $empresa_id) : [];
$eval_perfiles_cargo = $modulo_actual === 'evaluaciones-medicas' ? estandar5_perfiles_cargo_rows($conn, $empresa_id) : [];
$eval_trabajadores = $modulo_actual === 'evaluaciones-medicas' ? estandar5_trabajadores_medicos_rows($conn, $empresa_id) : [];
$eval_solicitudes = $modulo_actual === 'evaluaciones-medicas' ? estandar5_evaluaciones_medicas_rows($conn, $empresa_id) : [];
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .pyp-kpi { background:#fff; border:1px solid var(--border); border-radius:10px; padding:13px; min-height:92px; box-shadow:0 8px 24px rgba(15,23,42,.04); }
        .pyp-kpi span { display:block; color:#64748b; font-size:.62rem; font-weight:850; text-transform:uppercase; }
        .pyp-kpi strong { display:block; color:var(--blue-dark); font-size:1.32rem; margin-top:8px; line-height:1; }
        .pyp-kpi small { display:block; color:#64748b; margin-top:6px; line-height:1.35; }
        ...
        .med-kpi { background:#fff; border:1px solid var(--border); border-radius:10px; padding:12px; box-shadow:0 8px 24px rgba(15,23,42,.04); }
        .med-kpi span { display:block; color:#64748b; font-size:.58rem; font-weight:850; text-transform:uppercase; }
        .med-kpi strong { display:block; margin-top:7px; color:var(--blue-dark); font-size:1.15rem; }
...
<main class="main-wrapper">
    ...
    <div class="content-area">
        ...
        <?php elseif ($modulo_actual === 'promocion-prevencion'): ?>
            ...
            <?php if (empty($pyp_rows)): ?>
                <div class="pyp-empty">
                    ...
                    <strong>No hay campañas PyP registradas todavía.</strong>
                    <p>Cuando programes una actividad con tipo <strong>Campaña PyP en Salud</strong>, aparecerá aquí agrupada por categoría.</p>
                </div>
            <?php else: ?>
        ...
        <?php elseif ($modulo_actual === 'perfiles-cargo'): ?>
            ...
            <section class="cargo-summary compact">
        ...
        <?php elseif ($modulo_actual === 'evaluaciones-medicas'): ?>
            ...
            <section class="med-summary">
                <article class="med-kpi"><span>Trabajadores</span><strong><?php echo count($eval_trabajadores); ?></strong></article>
                <article class="med-kpi"><span>Centros médicos</span><strong><?php echo count($eval_centros_medicos); ?></strong></article>
                <article class="med-kpi"><span>Perfiles de cargo</span><strong><?php echo count($eval_perfiles_cargo); ?></strong></article>
                <article class="med-kpi"><span>Solicitudes recientes</span><strong><?php echo count($eval_solicitudes); ?></strong></article>
            </section>

            <?php if (empty($eval_centros_medicos) || empty($eval_perfiles_cargo)): ?>
            ...
            <section class="med-card">
                <div class="med-card-head">
                    <div>
                        ...
                        <p>Selecciona perfil de cargo y centro médico. El sistema enviará la solicitud al correo registrado del centro con el PDF del perfil adjunto.</p>
                    </div>
                    <i class="fa-solid fa-stethoscope" style="color:var(--primary2);"></i>
                </div>
                        ...
                        <table class="med-table">
                            ...
                            <tbody>
                                <?php foreach ($eval_trabajadores as $trabajador): ?>
                                    <tr>
                                        <td>
                                            <span class="med-worker-name"><?php echo htmlspecialchars(trim($trabajador['nombre'] . ' ' . $trabajador['apellido'])); ?></span>
                                            <span class="med-muted">C.C. <?php echo htmlspecialchars($trabajador['cedula']); ?><br><?php echo htmlspecialchars($trabajador['email']); ?></span>
                                        </td>
                                        <td><?php echo htmlspecialchars(estandar5_eval_cargo_trabajador($trabajador)); ?></td>
                                        <td>
                                            <?php if (!empty($trabajador['ultima_eval_estado'])): ?>
                                                <span class="med-badge <?php echo htmlspecialchars($trabajador['ultima_eval_estado']); ?>"><?php echo htmlspecialchars($trabajador['ultima_eval_estado']); ?></span><br>
                                                <span class="med-muted"><?php echo htmlspecialchars($trabajador['ultimo_tipo_examen'] ?? ''); ?> · <?php echo htmlspecialchars(date('d/m/Y', strtotime($trabajador['ultima_eval_fecha']))); ?></span>
                                            <?php else: ?>
                                                <span class="med-muted">Sin solicitudes registradas</span>
                                            <?php endif; ?>
                                        </td>
                                        <td>
                                            <form class="med-form-grid" action="procesar_estandar5.php" method="POST">
                                                <input type="hidden" name="accion" value="programar_evaluacion_medica">
                                                <input type="hidden" name="trabajador_id" value="<?php echo (int)$trabajador['id']; ?>">
                                                <select class="med-field" name="tipo_examen" required <?php echo $can_program ? '' : 'disabled'; ?>>
                                                ...
                                                <select class="med-field" name="perfil_cargo_id" required <?php echo $can_program ? '' : 'disabled'; ?>>
                                                    <option value="">Perfil de cargo...</option>
                                                    <?php foreach ($eval_perfiles_cargo as $perfil): ?>
                                                        <option value="<?php echo (int)$perfil['id']; ?>"><?php echo htmlspecialchars($perfil['nombre_cargo'] . ' · ' . $perfil['tipo_operacion']); ?></option>
                                                    <?php endforeach; ?>
                                                </select>
                                                <select class="med-field" name="centro_medico_id" required <?php echo $can_program ? '' : 'disabled'; ?>>
                                                    <option value="">Centro médico...</option>
                                                    <?php foreach ($eval_centros_medicos as $centro): ?>
                                                        <option value="<?php echo (int)$centro['id']; ?>">
                                                            <?php echo htmlspecialchars($centro['nombre'] . ' · ' . $centro['direccion_principal'] . ' · ' . $centro['correo']); ?>
                                                        </option>
                                                    <?php endforeach; ?>
                                                </select>
                                                <input class="med-field" name="observaciones" placeholder="Observaciones">
                                                <button class="med-btn" type="submit" <?php echo $can_program ? '' : 'disabled'; ?>>
                                                    <i class="fa-solid fa-paper-plane"></i> Programar
                                                </button>
                                            </form>
                                        </td>
                                    </tr>
            ...
            <section class="med-card">
                <div class="med-card-head">
                    <div>
                        ...
                        <p>Historial básico de correos enviados a centros médicos desde este subestándar.</p>
                    </div>
                </div>
                        ...
                        <table class="med-table">
                            ...
                            <tbody>
                                <?php foreach ($eval_solicitudes as $sol): ?>
                                    <tr>
                                        <td><?php echo htmlspecialchars(date('d/m/Y H:i', strtotime($sol['creado_en']))); ?></td>
                                        <td><?php echo htmlspecialchars(trim($sol['nombre'] . ' ' . $sol['apellido'])); ?><br><span class="med-muted">C.C. <?php echo htmlspecialchars($sol['cedula']); ?></span></td>
                                        <td><?php echo htmlspecialchars($sol['nombre_cargo']); ?></td>
                                        <td><?php echo htmlspecialchars($sol['centro_nombre']); ?><br><span class="med-muted"><?php echo htmlspecialchars($sol['correo_destino']); ?></span></td>
                                        <td><?php echo htmlspecialchars($sol['tipo_examen']); ?></td>
                                        <td><span class="med-badge <?php echo htmlspecialchars($sol['estado']); ?>"><?php echo htmlspecialchars($sol['estado']); ?></span></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
...
<?php if ($modulo_actual === 'perfiles-cargo'): ?>
<script>
