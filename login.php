...
if (isset($_GET['resend']) && !empty($_SESSION['usuario_temp'])) {
    ...
    $correoDestino = ($usuario['correo_seguridad'] ?? '') ?: $usuario['email'];
    $_SESSION['codigo_2fa'] = str_pad((string) random_int(0, 999999), 6, '0', STR_PAD_LEFT);
    $_SESSION['codigo_expira'] = time() + 300;
    $_SESSION['correo_censurado'] = maskEmail($correoDestino);

    $fullName = trim(($usuario['nombre'] ?? '') . ' ' . ($usuario['apellido'] ?? ''));
    [$ok, $msg] = send2FAEmail($correoDestino, $fullName !== '' ? $fullName : 'Usuario', $_SESSION['codigo_2fa']);
    ...
    if (empty($identifier)) {
        ...
        try {
            if (!empty($_SESSION['codigo_2fa']) && !empty($_SESSION['usuario_temp'])) {
                ...
                $stmt = $conn->prepare("
                    SELECT id, nombre, apellido, cedula, email, correo_seguridad, rol, telefono
                    FROM usuarios
                    WHERE cedula = :identifier
                       OR email = :identifier
                       OR correo_seguridad = :identifier
                    LIMIT 1
                ");
